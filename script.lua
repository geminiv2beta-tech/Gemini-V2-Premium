...
-- 업그레이드된 Desync (고속 물리 지터링 + 서버 잔상 유지)
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= player and p.Character and p.Character:FindFirstChild(targetPartName) then
            ...
            if hum and hum.Health > 0 then
                local screenPos, onScreen = cam:WorldToViewportPoint(p.Character[targetPartName].Position)
...
-- 고개 숙임 방지 및 정밀 공격 보정 업그레이드 Void Spam + 보이드 스팸 중 총알 적중 100% 보정
RunService.Heartbeat:Connect(function()
    if isVoidSpam and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        local root = player.Character.HumanoidRootPart
        local currentCF = root.CFrame
        -- CFrame 회전각(시선 방향)을 완전히 유지하면서 Y축 위치만 순간 이동시켜 피격점을 지우고 고개 숙임 방지
...
pcall(function()
    ...
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if self == workspace and method == "Raycast" and isVoidSpam then
            local args = {...}
            local origin = args[1]
            if origin and origin.Y < -1000 then
                local target = getRageTarget() or (getTargetInFOV() and (getTargetInFOV():FindFirstChild("Head") or getTargetInFOV():FindFirstChild("HumanoidRootPart")))
                if target then
...
RunService:BindToRenderStep("AimbotLogic", Enum.RenderPriority.Camera.Value + 1, function()
    ...
    if isRageBot then
        ...
        if rageTarget then cam.CFrame = CFrame.new(cam.CFrame.Position, rageTarget.Position) end
    elseif isAimbot then
        local target = getTargetInFOV()
        local targetPartName = isAimingHead and "Head" or "HumanoidRootPart"
        if target and target:FindFirstChild(targetPartName) then cam.CFrame = CFrame.new(cam.CFrame.Position, target[targetPartName].Position) end
    end
end)
