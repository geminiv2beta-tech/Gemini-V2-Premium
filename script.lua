...
local function getDynamicSessionToken()
    return randomString(12) .. "=" .. randomString(12)
end
...
pcall(function()
    ...
    _stbl = hookfunction(getrenv().setmetatable, newcclosure(function(tbl, mt)
        if mt and typeof(mt) == "table" and rawget(mt, "__mode") == "kv" then
            ...
            if tr and (tr:find("MiscellaneousController") or tr:find("anticheat") or tr:find("Detection") or tr:find("Security") or tr:find("AntiExploit") or tr:find("Integrity")) then
                return _stbl({1, 2, 3}, {})
            end
        end
        return _stbl(tbl, mt)
    ...
    if hookmetamethod then
        ...
        oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
            ...
            if method == "FireServer" and self and self.Name and (self.Name:lower():find("anticheat") or self.Name:lower():find("ban") or self.Name:lower():find("report")) then
                return
            end
...
task.spawn(function()
    pcall(function()
        local _tags = {"anticheat", "ac", "detection", "ban", "kick", "security", "moderation", "antishot", "exploit", "integrity", "report"}
...
local function buildRaycastParams(targetSubject)
    local rp = RaycastParams.new()
    rp.FilterType = Enum.RaycastFilterType.Exclude
    rp.IgnoreWater = true
    rp.FilterDescendantsInstances = { lplr.Character, targetSubject }
    return rp
end

local function isClearLine(origin, target, rp)
    local dir = target - origin
    if dir.Magnitude <= 0 then return false end
    return ws:Raycast(origin, dir, rp) == nil
end

local function resolveShotOrigin(targetPart, targetPos, cameraPos)
    if not getgenv().Config.Wallbang then return cameraPos end
    local subject = targetPart:FindFirstAncestorOfClass('Model')
    local rp = buildRaycastParams(subject)

    if isClearLine(cameraPos, targetPos, rp) then
        return cameraPos
    end
    for _, dir in ipairs(SHELL_DIRECTIONS) do
        local candidate = cameraPos + dir * 9.5
        if isClearLine(candidate, targetPos, rp) then
            return candidate
        end
    end
    return cameraPos
...
local function showHitNotification(targetName, damageAmount, isVoidKill)
    ...
    pcall(function()
        ...
        if isVoidKill then
            notifLabel.Text = string.format("[보이드 처치] %s을(를) 보이드에서 죽임! (데미지: %d)", targetName, damageAmount)
        else
            notifLabel.Text = string.format("(%s 처치중. 입힌 데미지: %d)", targetName, damageAmount)
        end
...
runS.RenderStepped:Connect(function()
    ...
    if targetPlayer and targetHead then
        ...
        centerTextLabel.Text = string.format("(보이드 레이지봇이 %s 처치 중)", targetPlayer.Name)
    else
        centerTextLabel.Visible = false
    end
end)
...
runS.RenderStepped:Connect(function()
    ...
    if (getgenv().Config.BodyTeleport or getgenv().Config.VoidSpam or getgenv().Config.DesyncView) and targetRoot then
        pcall(function()
            local localChar = lplr.Character
            local localRoot = localChar and localChar:FindFirstChild("HumanoidRootPart")
            if localRoot then
                local rx = (math.random(-getgenv().Config.VoidRangeX, getgenv().Config.VoidRangeX)) * 0.05
                local ry = getgenv().Config.VoidBaseY + (math.random(0, getgenv().Config.VoidRangeY)) * 0.05
                local rz = (math.random(-getgenv().Config.VoidRangeZ, getgenv().Config.VoidRangeZ)) * 0.05
                
                local customTargetPos = targetRoot.Position + Vector3.new(rx, ry, rz)
                
                localRoot.CFrame = CFrame.new(customTargetPos)
    ...
    if getgenv().Config.Aimbot and not getgenv().Config.DesyncView then
        local cam = ws.CurrentCamera
    ...
    local isShootRequested = getgenv().Config.RageBot or getgenv().Config.AutoFire or getgenv().Config.RapidFire or getgenv().Config.SilentAim or getgenv().Config.Triggerbot or getgenv().Config.VoidSpam
    if isShootRequested then
        local currentDelay = math.clamp(getgenv().Config.AttackTime, 0.0001, 0.10)
        if getgenv().Config.RapidFire or getgenv().Config.VoidSpam then
            currentDelay = math.min(currentDelay, 0.0001)
        end
        ...
        if (getgenv().Config.Prediction or getgenv().Config.RageBot or getgenv().Config.SilentAim) and targetRoot then
            predictedPos = predictedPos + (targetRoot.AssemblyLinearVelocity * getgenv().Config.HeightTime)
        end
        ...
        local origin = resolveShotOrigin(targetHead, targetPos, camPos)
        ...
        pcall(function()
            ...
            local originStruct = {
                [utf8.char(0)] = origin.X, [utf8.char(1)] = origin.Y, [utf8.char(2)] = origin.Z,
                [utf8.char(3)] = lX,       [utf8.char(4)] = lY,       [utf8.char(5)] = lZ,
            }

            local rCF = objSpaceHeadOffset
            local rX, rY, rZ = rCF:ToOrientation()
            ...
            cameradata[utf8.char(1)] = {
                ...
                [utf8.char(3)] = {
                    [utf8.char(0)] = rCF.X, [utf8.char(1)] = rCF.Y, [utf8.char(2)] = rCF.Z,
...
local function updateFly(state)
    ...
    if flying then
        flyConnection = runS.RenderStepped:Connect(function()
            ...
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if root then
                root.Velocity = Vector3.new(0, 1, 0) * (getgenv().Config.FlySpeed or 70)
            end
        end)
    else
...
titleBar.Text = "RayV8 Ultra Gold // Desync, Void Teleport & All Skins" 
...
local combatSec1 = createSection(cLeft, "Combat & Ragebot (Players Only)")
local combatSec2 = createSection(cRight, "Desync & Wallbang")
...
createToggle(combatSec1, "Triggerbot (Instant Players)", 4, getgenv().Config.Triggerbot, function(v) getgenv().Config.Triggerbot = v; getgenv().Config.Enabled = v end)
createToggle(combatSec1, "All-Head (Forced)", 5, getgenv().Config.AllHead, function(v) getgenv().Config.AllHead = v end)

createToggle(combatSec2, "Advanced Wallbang (벽 관통)", 1, getgenv().Config.Wallbang, function(v) getgenv().Config.Wallbang = v end)
createToggle(combatSec2, "Body Teleport", 2, getgenv().Config.BodyTeleport, function(v) getgenv().Config.BodyTeleport = v end)
createToggle(combatSec2, "Desync View (디싱크 적용)", 3, getgenv().Config.DesyncView, function(v) getgenv().Config.DesyncView = v end)
...
createToggle(miscSec1, "플레이어 대미지 / 보이드 처치 텍스트", 1, getgenv().Config.PlayerDamageText, function(v) getgenv().Config.PlayerDamageText = v; getgenv().Config.HitNotify = v end)
