-- Disabled for stability; some executors can crash Roblox when changing thread identity at startup.
-- native disabled for obfuscated builds.
--!strict
...
lionGetGroup("main", "weapons", "right")
    ...
    -- Floating always-visible Open/Close menu button (vallkmult Premium v3)
    do
        local toggleBtnGui = Instance.new("ScreenGui")
        toggleBtnGui.Name = "VallkmultMenuToggle"
        toggleBtnGui.ResetOnSpawn = false
        toggleBtnGui.IgnoreGuiInset = true
        toggleBtnGui.DisplayOrder = 9999
        ...
        toggleBtnGui.Parent = guiParent

        local btn = Instance.new("TextButton")
        btn.Name = "MenuToggleButton"
        btn.Size = UDim2.fromOffset(118, 34)
        btn.Position = UDim2.new(0, 12, 0.5, -17)
        ...
        local function refreshToggleButton()
            ...
            pcall(function()
                if LionWindow and LionWindow.Holder then
                    ...
                    open = mainapi.ClickGuiStatus == true
                end
            end)
            btn.Text = open and "CLOSE" or "MENU"
            btn.BackgroundColor3 = open and Color3.fromRGB(90, 35, 55) or Color3.fromRGB(28, 28, 36)
            stroke.Color = open and Color3.fromRGB(255, 90, 130) or Color3.fromRGB(180, 120, 255)
        end

        local function doToggleMenu()
            local toggled = false
            pcall(function()
                if LionLibrary and type(LionLibrary.Toggle) == "function" then
                    LionLibrary:Toggle()
                    toggled = true
                end
            end)
            if not toggled then
                pcall(function()
                    if LionWindow and LionWindow.Holder then
                        LionWindow.Holder.Visible = not LionWindow.Holder.Visible
                        mainapi.ClickGuiStatus = LionWindow.Holder.Visible
                        setMenuBlur(mainapi.ClickGuiStatus)
                    else
            ...
            task.defer(refreshToggleButton)
        end

        btn.MouseButton1Click:Connect(doToggleMenu)

        -- Keep label in sync with actual window state
        mainapi:Clean(RunService.Heartbeat:Connect(function()
            refreshToggleButton()
        end))

        -- Make button draggable so it never blocks gameplay
...
local function setRagebotStatus(enabled, target, voiding)
    ...
    if enabled and target and not voiding then
        text = "Ragebot : killing " .. (target.Name or "target")
    end
...
run(function()
    ...
    Ragebot:AddDropdown({
        ...
        Function = function(value)
            ...
            RagebotSettings.primarySlot = value == 'primary' and 1 or 2
            RagebotSettings.secondarySlot = value == 'primary' and 2 or 1
            markRagebotSettingsDirty()
        end
    })
